    ...
    *,
    ...
    for desired_idx, desired_item in enumerate(desired_items):
        ...
        if prefer_name_matching:
            current_idx = _first_unmatched_index(
    ...
    for raw_item in cast(list[object], raw_items):
        ...
        if prefer_name_matching:
            ...
            if isinstance(name, str) and name:
                idx = _first_unmatched_index(by_name.get(name, []), unmatched_current)
        else:
            raw_id = item.get("id")
            ...
            if idx is None:
                name = item.get("name")
                if isinstance(name, str) and name:
                    idx = _first_unmatched_index(
                        by_name.get(name, []), unmatched_current
                    )
        if idx is None:
            continue
        defined.add(idx)
